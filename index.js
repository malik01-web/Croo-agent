...
if (!sdkKey) {
  console.error("FATAL: CROO_SDK_KEY is completely missing from your environment.");
  process.exit(1);
}
if (!GEMINI_API_KEY) {
  ...
  try {
    ...
    stream.on(EventType.OrderPaid, async (event) => {
      ...
      try {
          ...
          if (!urlMatch) {
             console.error("[WARN] Extraction failed. Could not find a URL in the payload.");
          ...
          console.log("Analyzing compressed text via Gemini...");
